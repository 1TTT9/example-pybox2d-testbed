...
class Framework(object):
    ...
    * Handles all user input.
    ...
    bomb               = None
    mouseJoint         = None
    mouseJointGroup    = []  # defined 2013/07/30
    settings           = fwSettings
    bombSpawning       = False
    bombSpawnPoint     = None
    mouseWorld         = None
    mouseWorldGroup    = [] # defined 2013/07/30
    destroyList        = []
    ...
    def Step(self, settings):
        ...
        if settings.drawStats:
            ...
            self.DrawStringCR("ball/player(kg) = %d/%d" % (self.master.GetMass(),self.players[0].GetMass()) )
        ...
        if not self.enableMU:
            ...
            for mj in self.mouseJointGroup:
                if mj:
                    body = mj.GetBody2()
                    p1 = body.GetWorldPoint(mj.localAnchor)
                    p2 = mj.target
        ...
        if self.bombSpawning:
            ...
            if not self.enableMU:
                ...
                for mw in self.mouseWorldGroup:
                    self.debugDraw.DrawSegment(self.bombSpawnPoint, mw, (0.8,0.8,0.8))
    ...
    def MultipleMouseDown(self, i, p):
        """
        Indicates that there was a left click at point p (world coordinates)
        """

        if self.mouseJointGroup[i] != None:
            return
        ...
        if body:
            ...
            md.target  = p
            md.maxForce= 1000.0 * body.GetMass()
            self.mouseJointGroup[i] = self.world.CreateJoint(md)
            body.WakeUp()
    ...
    def MultipleMouseUp(self, i, p):
        """
        Left mouse button up.
        """     
        if self.mouseJointGroup[i]:
            self.world.DestroyJoint(self.mouseJointGroup[i])
            self.mouseJointGroup[i] = None
            self.mouseWorldGroup[i] = None
    ...
    def MultipleMouseMove(self, i, p):
        """
        Mouse moved to point p, in world coordinates.
        """
        self.mouseWorld = p
        self.mouseWorldGroup[i] = p
        if self.mouseJointGroup[i]:
            self.mouseJointGroup[i].SetTarget(p)
    ...
    def ConvertScreenToWorld2(self, x, y):
        """
        Return a b2Vec2 in world coordinates of the passed in screen coordinates x, y
        """
        return box2d.b2Vec2( (x + self.viewOffset.x) / self.viewZoom, 
                           ((self.screenSize.y - y + self.viewOffset.y) / self.viewZoom + 4.0))


    def ConvertWorldToScreen(self, vec):
        """
        Return a tuple in screen coordinates of the passed in world coordinates b2Vec2
        """
        return tuple( map(int, ( vec.x * self.viewZoom - self.viewOffset.x,  
                                 self.screenSize.y + self.viewOffset.y - vec.y * self.viewZoom )) )


    def ConvertWorldToScreen2(self, vec):
        """
        Return a tuple in screen coordinates of the passed in world coordinates b2Vec2
        """
        return tuple( map(int, ( vec.x * self.viewZoom - self.viewOffset.x,
                                 self.screenSize.y + self.viewOffset.y - vec.y * self.viewZoom + 4.0*self.viewZoom)) )
